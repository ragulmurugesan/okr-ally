...
export default function OkrSegment({ count, title, nestChildren = [] }) {
    ...
    return (
        <div className="ally-okr__segment">
            ...
            {show &&
                <ul>
                    {nestChildren.length ? nestChildren.map(child => <li key={child.id} className="ally-okr__segment-item">{child.title}</li>) : <li className="ally-okr__segment-item">No data found.</li>}
                </ul>
            }
        </div>
    )
